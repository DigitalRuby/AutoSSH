...
Backups download over eight SFTP connections per host, each keeping eight downloads in flight (64 concurrent files per host), plus a separate connection for directory scanning. Small files cost several network round trips each, so concurrency rather than bandwidth sets the speed on high-latency links. Up to eight hosts are processed concurrently with asynchronous task coordination. Directory scanning overlaps downloads: on Linux the file list comes from one remote `find` command instead of an SFTP round trip per folder; Windows (or Linux without GNU `find`) walks directories over SFTP. The scanner connection only lists files when `find` is unavailable, and transfer workers start as soon as changed files are found. Sync reuses listing metadata to skip unchanged files without extra per-file metadata requests, compares timestamps at whole-second precision so file systems with coarse timestamps don't force re-downloads, and opens download connections only when files need downloading. Uploads remain sequential and asynchronous within each host's SFTP connection, caching remote directories already created. SSH connections send keepalives every 15 seconds. Connection and login/sudo prompt waits time out after 30 seconds; SFTP operations time out after 60 seconds, and shell commands after 30 minutes. A connection or operation timeout stops further work on that host and reports an error while other hosts continue. Run the command file again to retry; completed backups are skipped using their timestamps.

Set `AUTOSSH_DOWNLOAD_WORKERS` to a whole number from 1 to 16 to change the number of download connections per host (default 8). Set it to 1 to use only the original SFTP connection for sequential transfers.

Set `AUTOSSH_DOWNLOADS_PER_CONNECTION` to a whole number from 1 to 64 to change how many downloads each download connection runs at once (default 8).
